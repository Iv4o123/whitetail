...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    window->DrawList->AddRectFilled(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(255, 255, 255), 3.0f);
    window->DrawList->AddRect(total_bb.Min, total_bb.Min + ImVec2(square_sz, square_sz), ImColor(200, 200, 200), 3.0f);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 15));
    ...
    static float slide;
    slide = ImClamp(slide + (1.f * ImGui::GetIO().DeltaTime * (style.Alpha == 1 ? 1.5f : -3.f)), 0.0f, 1.f);

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max - ImVec2(9, 0), ImColor(230, 230, 230), 5.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(21, 112, 232), 5.0f);
    window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x - 1, grab_bb.Min.y + 1), 6.0f, ImColor(21, 112, 232), 30);

    RenderText(total_bb.Min, label);

    PushStyleColor(ImGuiCol_Text, ImVec4(0.0f, 0.0f, 0.0f, it_anim->second.opacity)); {
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        ImGui::PushStyleColor(ImGuiCol_PopupBg, ImVec4(ImColor(250, 250, 250)));
        ImGui::PushStyleColor(ImGuiCol_Border, ImVec4(ImColor(24, 24, 24, 15)));
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(size, 15));
    const ImRect clickable(rect.Max - ImVec2(15, 15), rect.Max);
    ...
    //window->DrawList->AddText(rect.Min + ImVec2(0, 4), ImColor(24, 24, 24, 24), desc_id);
    //window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(24, 24, 24), 3.f, 14.f);
    if (col_rgb.w < 1.0f)
...
bool ImGui::MenuTabEx(const char* label, const bool active, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = g.Font->CalcTextSizeA(g.Font->FontSize, FLT_MAX, 0, label, NULL);
    ...
    it_active->second = ImClamp(it_active->second + (2.f * ImGui::GetIO().DeltaTime * (active ? 1.f : -1.f)), 0.0f, 1.0f);
    ...
    window->DrawList->AddRectFilled(ImVec2(pos.x, pos.y + size_arg.y - 2), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(141, 157, 255, int(255 * it_active->second)));
    window->DrawList->AddRectFilledMultiColor(ImVec2(pos.x, pos.y + size_arg.y - 15), ImVec2(pos.x + size_arg.x, pos.y + size_arg.y), ImColor(141, 157, 255, 0), ImColor(141, 157, 255, 0),
        ImColor(141, 157, 255, int(100 * it_active->second)), ImColor(141, 157, 255, int(100 * it_active->second)));

    PushStyleColor(ImGuiCol_Text, ImColor(75, 75, 75, int(255 * fill)).operator ImVec4());
...
bool ImGui::MenuTab(const char* label, const bool active, const ImVec2& size_arg)
{
    return MenuTabEx(label, active, size_arg, 0);
}
